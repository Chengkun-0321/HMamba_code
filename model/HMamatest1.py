...
# 輸出為 Concatenate 後的張量，shape 為 (batch, 3)，分別為 mean, upper, lower 分支
# ---------------------------
def build_model(args: ModelArgs):
    ...
    # 注意：這裡原始程式將輸出做了轉置與 expand_dims，故最終 output shape 為 (3, 1, batch)

    print("Model output shape (for debug):", output)
    
    adam = keras.optimizers.Adam(learning_rate=0.001)
    # 用我們自訂的 loss_mse 來編譯模型
...
def loss_mse_lambda(outputs, targets, lam):
    ...
    """
    squared_error = tf.square(outputs - targets)
    weighted_loss = lam * squared_error
    sum_loss = tf.reduce_sum(weighted_loss)
    count = tf.reduce_sum(tf.cast(lam > 0.0, tf.float32))
...
def loss_mse_lambda_2(outputs, outputs2, targets, lam):
    """
    計算加權的 mse 差異 loss
    outputs, outputs2, targets, lam 均為 shape (batch, 1)
    """
    mse_diff = tf.square(outputs - targets) - tf.square(outputs2 - targets)
    weighted_loss = lam * mse_diff
    sum_loss = tf.reduce_sum(weighted_loss)
    count = tf.reduce_sum(tf.cast(lam > 0.0, tf.float32))
    count = tf.maximum(count, 1.0)
    return sum_loss / count
...
args = ModelArgs(
    ...
    dropout_rate=0.2,
    num_classes=1,
    loss=loss_mse  # 雖然此處傳入 loss 參數，但我們在 build_model 裡面直接指定了 loss 函數
)
...
# ---------------------------
epochs = 15
history = model.fit(train_X_2d, train_Y_2d, batch_size=100, epochs=epochs,
                    validation_data=(valid_X_2d, valid_Y_2d), validation_freq=5)

# ---------------------------
# 預測與評估
# ---------------------------
pred_y = model.predict(train_X_2d)  # 預測結果 shape 為 (num_samples, ?)
...
# 將 train_Y_2d 先轉成 numpy 陣列以避免 RaggedTensor 無法取得長度的問題
