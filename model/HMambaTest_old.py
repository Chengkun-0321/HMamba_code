...
# --------------------------------
# 主程式區塊：利用命令列參數載入測試資料、checkpoint 與其他邊界參數
# --------------------------------
if __name__ == '__main__':
    ...
    # 設定分類用參數：mean_range 根據 mean 與 theta（此處 theta 固定為 2.0）
    theta = 2.0
    mean_range_upper = mean + theta
    mean_range_lower = mean - theta

    # 利用 mean 分支預測結果作為 classifier
    pred_test_net_cls = np.copy(pred_y[0])
    for idx, data in enumerate(pred_test_net_cls):
        if mean_range_lower <= data.item() <= mean_range_upper:
            pred_test_net_cls[idx] = 0
        elif data.item() > mean_range_upper:
            pred_test_net_cls[idx] = 1
        elif data.item() < mean_range_lower:
            pred_test_net_cls[idx] = 2
    pred_test_net_cls = tf.squeeze(pred_test_net_cls).numpy().astype('int64')

    # 整合 RACNN 與 Classifier 的預測結果
