...
def loss_mse(y_true, y_pred):
    ...
    for i in range(len(y_pred)):
        if i == 0:  # 均
            ...
            lambda_base = tf.where(lambda_ > theta, 1.0, 0.0)
            
            # 計算 lambda_ 的標準化
            a = tf.math.tanh(lambda_)
            lambda_ = tf.where(y_true >= 70.0, 1.0, a)
            
            total_lambda = lambda_base + lambda_
            
            l = loss_mse_lambda(y_pred[1], y_true, total_lambda)
            l += tf.clip_by_value(loss_mse_lambda_2(y_pred[1], y_pred[0], y_true, total_lambda), 
            ...
            # 計算 lambda_ 的標準化
            a = tf.math.tanh(lambda_)
            lambda_ = tf.where(y_true <= 60.0, 1.0, a)
            
            total_lambda = lambda_base + lambda_
            
            l = loss_mse_lambda(y_pred[2], y_true, total_lambda)
            l += tf.clip_by_value(loss_mse_lambda_2(y_pred[2], y_pred[0], y_true, total_lambda), 
                                  clip_value_min=0.0, clip_value_max=10000.0)
            _loss_mse.append(l)
            loss_mse += l
...
# MAPE (Mean Absolute Percentage Error, 平均絕對百分比誤差) - 使用 numpy
...
def MAPE(y_true, y_pred):
    ...
    nonzero_mask = tf.not_equal(y_true, 0)  # 防止除以零的情況
    y_true_safe = tf.where(nonzero_mask, y_true, tf.ones_like(y_true))  # 將零值變為 1 確保除法不會出錯
    
    mape = tf.reduce_mean(tf.abs((y_true - y_pred) / y_true_safe)) * 100
    return mape
'''
# MAE (使用 numpy)
...
def plot_training_history(history, save_path = None):
    ...
    if 'custom_accuracy' in history.history:
        ...
        if 'val_custom_accuracy' in history.history:
